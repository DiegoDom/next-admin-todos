...
export const TopMenu = async () => {
  ...
  const itemsTotal = getTotalCount(cart);
      ...
      <div className="px-6 flex items-center justify-between space-x-4">
        ...
        <div className="flex space-x-2">
          ...
          <Link href={"/dashboard/cart"} className="p-2 flex items-center justify-center h-10 rounded-xl border bg-gray-100 focus:bg-gray-100 active:bg-gray-200">
            {itemsTotal > 0 && itemsTotal < 100 && <span className="text-sm mr-2 text-blue-800 font-bold">{itemsTotal}</span>}
            {itemsTotal >= 100 && <span className="text-sm mr-2 text-red-800 font-bold">99+</span>}
